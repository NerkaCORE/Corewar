...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/02/10 18:31:03 by gduval            #+#    #+#             */
/*   Updated: 2018/02/10 18:31:08 by gduval           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		init_pair_by_id(t_data *data)
{
	...
	while (cpt < data->nb_champ)
	{
		...
		if (cpt == 0)
			...
			init_pair(i % 255, COLOR_MAGENTA, COLOR_BLACK);
		else if (cpt == 3)
...
static int		getch_aff(t_data *data)
{
	...
	mvwprintw(data->display->screen, 44, (194 / 2) - (58 / 2),
			"Press Space to run the game or S to run the slowmode.");
...
int				aff_window(t_data *data)
{
	...
	if (!(display = (t_display*)ft_memalloc(sizeof(t_display))))
		return (0);
	*display = (t_display){NULL, NULL, NULL, NULL, NULL};
	...
	{
		ft_printf(
		"\nToo short screen size too display :\nLINES MIN: 81 / Value TTY: ");
